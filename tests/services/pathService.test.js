...
describe('PathService', () => {
  ...
  describe('getPdfPath', () => {
    ...
    test('应该处理没有文件名的URL', () => {
      ...
      expect(pdfPath).toBe(
        path.join(mockConfig.pdfDir, 'nextjs.org-docs', '001-nextjs-org.pdf')
      );
    });
    ...
    test('应该在没有useHash和index时使用简单文件名', () => {
      ...
      expect(pdfPath).toBe(
        path.join(mockConfig.pdfDir, 'nextjs.org-docs', 'a1b2c3d4-routing.pdf')
      );
    });
  });
  ...
  describe('getMetadataPath', () => {
    test('应该返回articleTitles元数据路径', () => {
      ...
      expect(metaPath).toBe(
        path.join(mockConfig.pdfDir, 'metadata', 'articleTitles.json')
      );
    });
    ...
    test('应该返回failed元数据路径', () => {
      ...
      expect(metaPath).toBe(
        path.join(mockConfig.pdfDir, 'metadata', 'failed.json')
      );
    });
    ...
    test('应该返回imageLoadFailures元数据路径', () => {
      ...
      expect(metaPath).toBe(
        path.join(mockConfig.pdfDir, 'metadata', 'imageLoadFailures.json')
      );
    });
    ...
    test('应该返回progress元数据路径', () => {
      ...
      expect(metaPath).toBe(
        path.join(mockConfig.pdfDir, 'metadata', 'progress.json')
      );
    });
    ...
    test('应该返回urlMapping元数据路径', () => {
      ...
      expect(metaPath).toBe(
        path.join(mockConfig.pdfDir, 'metadata', 'urlMapping.json')
      );
    });

    test('应该为未知类型抛出错误', () => {
      expect(() => pathService.getMetadataPath('unknown')).toThrow(
        '未知的元数据类型: unknown'
      );
    });
  });
  ...
  describe('getFinalPdfPath', () => {
    test('应该生成带日期的最终PDF路径', () => {
      ...
      expect(finalPath).toBe(
        path.join(mockConfig.pdfDir, 'finalPdf', 'nextjs-docs_20240315.pdf')
      );
  ...
  describe('getTempPath', () => {
    test('应该返回临时文件路径', () => {
      ...
      expect(tempPath).toBe(
        path.join(mockConfig.pdfDir, '.temp', 'temp-file.txt')
      );
    });
  });
